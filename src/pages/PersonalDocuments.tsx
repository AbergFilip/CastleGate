...
const subcategories = {
  visa: 'Visum',
  gift_letters: 'Gåvobrev',
  wills: 'Testamente',
  prenuptial: 'Äktenskapsförord',
}
...
function PersonalDocuments() {
  ...
  const loadDocuments = async () => {
    try {
      setLoading(true)
      setError(null)
      const allDocuments = await getDocuments({ category: 'personal' })
  ...
  const handleDocumentClick = (document: any) => {
    // Navigera till dokumentdetaljer eller öppna fil
  ...
  return (
    <div style={{ background: '#F4F6FF', minHeight: '100vh', width: '100%', position: 'relative' }}>
      ...
      >
        {loading ? (
          ...
          <>
            <Section
              title="Visum"
              items={documents.visa}
              onItemClick={handleDocumentClick}
              onItemDelete={handleDeleteDocument}
              onAdd={() => handleAddDocument('visa')}
            />
            <Section
              title="Gåvobrev"
              items={documents.gift_letters}
              onItemClick={handleDocumentClick}
              onItemDelete={handleDeleteDocument}
              onAdd={() => handleAddDocument('gift_letters')}
            />
            <Section
              title="Testamente"
              items={documents.wills}
              onItemClick={handleDocumentClick}
              onItemDelete={handleDeleteDocument}
              onAdd={() => handleAddDocument('wills')}
            />
            <Section
              title="Äktenskapsförord"
              items={documents.prenuptial}
              onItemClick={handleDocumentClick}
              onItemDelete={handleDeleteDocument}
              onAdd={() => handleAddDocument('prenuptial')}
            />
          </>
        )}
      </div>
      ...
      <Modal
        ...
        title={`Lägg till ${currentSubcategory ? subcategories[currentSubcategory as keyof typeof subcategories] : 'dokument'}`}
      >
        <FormField
          label="Titel"

...
function Vehicles() {
  ...
  const sections = [
    ...
    {
      title: 'Motorcykel',
      items: [{ name: 'Kawasaki Ninja Zx 10r', detail: 'LBT 412' }],
    },
    {
      title: 'Släp',
      items: [{ name: 'Freev EVR 1000', detail: 'Ej registrerat' }],
    },
    {
      title: 'Andra fordon',
          ...
          <section key={section.title} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {section.items.map((item) => {
                ...
                if (item.name === 'Volvo XC90') {
                  return (
                    <Link key={`${section.title}-${item.name}`} to="/properties/vehicles/volvo-xc90" style={{ textDecoration: 'none' }}>
                      {content}
                    </Link>
                  )
                }
